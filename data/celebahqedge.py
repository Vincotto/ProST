...
class CelebAHQEdgeDataset(Pix2pixDataset):
    def __init__(self, opt):
        super(CelebAHQEdgeDataset, self).__init__(opt)
    ...
    def get_paths(self, opt):
        ...
        for i in range(len(lines)):
            ...
            label_paths.append(os.path.join(opt.dataroot, 'CelebAMask-HQ-mask-anno', lines[i].strip().zfill(5) + '.png'))
    ...
    def get_label_tensor(self, path):
        ...
        tensors_dist = 0
        e = 1
        for part in ['skin', 'l_brow', 'r_brow', 'l_eye', 'r_eye', 'l_ear', 'r_ear', 'nose', 'u_lip', 'mouth', 'l_lip', 'eye_g', 'hair']:
    ...
    def imgpath_to_labelpath(self, path):
        ...
        label_path = os.path.join(root, 'CelebAMask-HQ-mask-anno', subfolder, name.split('.')[0].zfill(5) + '.png')
        return label_path
...
def main():
    dataroot = '/media/yangxilab/DiskB/xhy/pgbcc/pytorch-CycleGAN-and-pix2pix/datasets/deepfashion/'
    dataset = CelebAHQEdgeDataset(type('opt', (object,), {'dataroot': dataroot, 'load_size': 256}))

    base_dir = '/media/yangxilab/DiskB/xhy/pgbcc/pytorch-CycleGAN-and-pix2pix/datasets/deepfashion/pose/WOMEN/Sweaters/id_00002198'
    candidate_files = []
    for entry in os.listdir(base_dir):
        full_path = os.path.join(base_dir, entry)
        if os.path.isfile(full_path) and entry.endswith('candidate.txt'):
            label_path = full_path
            subset_path = label_path.replace('candidate', 'subset')
            img_path = dataset.labelpath_to_imgpath(label_path)
            pose = dataset.get_label_tensor(label_path)

            lable_filename = img_filename = os.path.basename(img_path)
            base_name = os.path.splitext(os.path.basename(dataset.labelpath_to_imgpath(label_path)))[0]
            img_filename = os.path.basename(img_path)
            img_dir = os.path.dirname(img_path)
            output_dir = img_dir.replace('img', 'out')

            if not os.path.exists(output_dir):
                os.makedirs(output_dir)

            # Save the edge image
            edge_image = pose[0].numpy().astype(np.uint8)
            cv2.imwrite(os.path.join(output_dir, 'edge_' + img_filename), edge_image)
